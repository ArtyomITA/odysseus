#!/usr/bin/env node
/** Read-only 7011 list/read tools followed by a referential refresh. */
...
const reportPath = path.resolve(process.env.REPORT_PATH || path.join(root, `reports/read-tool-followups-${new Date().toISOString().replace(/[:.]/g, '-')}.json`));
...
const cases = [
  { name: 'model-catalog', tool: 'list_models', prompts: ['List available models. Read only.', 'Refresh that same model catalog list. Read only.'] },
  { name: 'served-models', tool: 'list_served_models', prompts: ['List currently served models and their status. Read only.', 'Refresh that same served-model list. Read only.'] },
  { name: 'downloads', tool: 'list_downloads', prompts: ['List current Cookbook model downloads. Read only.', 'Refresh that same downloads list. Read only.'] },
  { name: 'serve-presets', tool: 'list_serve_presets', prompts: ['List saved Cookbook serve presets. Read only.', 'Refresh that same serve-preset list. Read only.'] },
  { name: 'cached-models', tool: 'list_cached_models', prompts: ['List locally cached models. Read only.', 'Refresh that same cached-model list. Read only.'] },
...
const report = { model, status: 'running', cases: [], privacy: 'No tool output, model names, hosts, paths, or answer text retained.' };
  ...
  for (const spec of cases) {
    ...
    try {
      const created = await context.request.post(`${base}/api/session`, { multipart: {
        name: `[read-tool-followup] ${spec.name}`, model, endpoint_id: endpointId,
      ...
      for (let index = 0; index < spec.prompts.length; index++) {
        ...
        const checks = {
          ...
          exactly_one_expected_call: starts.length === 1 && starts[0]?.tool === spec.tool,
        ...
        result.turns.push({ index, offered_expected: checks.expected_offered, tools: starts.map(event => event.tool), checks, status: Object.values(checks).every(Boolean) ? 'passed' : 'failed' });
      }
