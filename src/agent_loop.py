...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Vergilius chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed Vergilius internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    return untrusted_context_message(
        ...
        (
            "Saved user memory facts from Vergilius Brain. These are the same "
...
def _minimal_recent_notes_tool_context_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    return untrusted_context_message(
        "recent tool context",
        (
            "Recent Vergilius tool context for follow-up references only. "
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; Vergilius preserves everything from that separator downward automatically.\n'
...
async def stream_agent_loop(
    ...
    async def _build_route_request_state(candidate_url, candidate_model, candidate_headers, source_messages):
        ...
        route_messages, route_mcp_schemas = _build_system_prompt(
            ...
            workspace=workspace,
            include_mcp_descriptions=not is_api,
        )
    ...
    def _filter_route_tool_schemas(schemas):
        ...
        # Vergilius: ordine degli schemi fissato per il turno (prefisso KV).
        return _pin_schema_order(schemas)
    ...
    def _tool_schemas_for_route(route_state):
        ...
        if route_state["is_api_model"]:
            if route_relevant_tools:
                ...
                # Vergilius: senza una selezione esplicita non si mandano MAI
                # gli schemi Playwright grezzi; il modello vede solo gli
                # adattatori browser_* (browser_more li sblocca a richiesta).
        ...
        schemas = [
            schema for schema in (route_mcp_schemas if wants_mcp and route_mcp_schemas else [])
            if not (schema.get("function") or {}).get("name", "").startswith(_BROWSER_MCP_PREFIX)
        ]
        return _filter_route_tool_schemas(schemas)
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, block in enumerate(tool_blocks):
            ...
            if "ask_user" in result:
                ...
                # Eccezione: il varco di sicurezza (kind="tool_approval").  Li'
                # la domanda non e' del modello ma del guscio, vive solo nel
                # pannello di approvazione (tradotto in italiano) e il modello
                # non deve rileggersela nel turno dopo.  Scriverla anche nella
                # bolla la mostrava due volte, la seconda in inglese.
                if _auq.get("kind") == "tool_approval":
                    _auq_q = ""
