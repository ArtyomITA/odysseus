...
REGOLE_VISTA = (
    ...
    "- A Windows program the user names in Italian ('Blocco note', 'Calcolatrice', 'Esplora file', "
    "'Paint', 'Impostazioni') is a DESKTOP app: pass that exact Italian name to "
    "`mcp__windows__App(mode='launch', name='Blocco note')`, then type into it with "
    "`mcp__windows__Type`. Document, note and file tools live INSIDE Vergilius and are never the "
    "answer to 'apri il Blocco note': they would not touch the PC at all.\n"
    "- Windows 11 reopens the previous session of an editor, so 'Blocco note' can come up with the "
    "user's own file already loaded. Before typing into 'Blocco note' (or any editor you just "
    "launched), open a blank sheet with `mcp__windows__Shortcut(shortcut='ctrl+n')`, then look with "
    "`vista_schermo` and type only when the sheet is empty. Never type into a document you did not "
    "create, and never save it.\n"
    ...
    "- `loc` on every `mcp__windows__*` tool is ALWAYS a pair of screen pixels copied from `albero_ui`, "
    "e.g. `loc=[742, 318]`. A browser ref ('e4', 'ref_3'), an element name or a window title there makes "
    "the call fail with a JSON error. Without a usable (x,y), look again with `vista_schermo` or "
    "`vista_trova` instead of guessing.\n"
...
REGOLE_VISTA_LITE = (
    ...
    "- An Italian program name ('Blocco note', 'Calcolatrice', 'Esplora file', 'Paint') is a DESKTOP app: "
    "`App(mode='launch', name='Blocco note')` then `Type`. Document/note tools live inside Vergilius and "
    "never satisfy 'apri il Blocco note': they do not touch the PC.\n"
    "- Windows 11 reopens the previous session: a just-launched editor may already hold the user's own "
    "file. Before typing, `Shortcut(shortcut='ctrl+n')` for a blank sheet, then `vista_schermo` to check "
    "it is empty. Never type into a document you did not create, never save it.\n"
    ...
    "- `loc` on every `mcp__windows__*` tool is ALWAYS a pair of screen pixels copied from `albero_ui`, "
    "e.g. `loc=[742, 318]`. A browser ref ('e4', 'ref_3') or a name there makes the call fail with a JSON "
    "error. No usable (x,y) yet? Call `vista_schermo` again, or `vista_trova`, before acting.\n"
