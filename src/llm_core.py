...
async def _nonstream_tools_call(target_url: str, payload: Dict, headers, timeout, dump_path: Optional[str] = None):
    ...
    for c in (msg.get("tool_calls") or []):
        ...
        calls.append({"id": c.get("id") or "",
                      # upstream: gpt-oss/harmony tools go out renamed, so map
                      # the name back before handing it to the caller.
                      "name": _unalias_harmony_tool_name(f.get("name") or "", _model),
                      "arguments": args})
    if calls:
