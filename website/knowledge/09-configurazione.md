...
Lista autorevole delle chiavi: `DEFAULT_SETTINGS` in `src/settings.py`. File
salvato fuso sopra i predefiniti: chiave assente = predefinito. Cache di 2
secondi: **modifica a mano si applica quasi subito, senza riavvio**.
...
admin. Sottoinsieme sovrascrivibile per utente (`_PER_USER_KEYS`): modelli di
default, utility, vision, immagini, research. Resto ignora preferenza utente.
...
Scorciatoia: tool `manage_settings`. Si dice in chat "cambia canale reminder in
ntfy" e lo fa. Chiavi API restano di sola lettura.
...
`stt_provider: "stream"` è **nostro**, non di Odysseus: trascrizione in diretta
via WebSocket su `/api/stt/stream`. Per tornare al giro a lotti (Parakeet,
resta installato):
...
Varianti modello disponibili: 80, 160, 320, 560, 1120 ms — finestra che il
riconoscitore guarda. Più corta = meno ritardo, un po' meno precisione. Noi
usiamo **320 ms**.

## Endpoint dei modelli

Non stanno in `.env`: righe della tabella `model_endpoints` del database.
...
1. **chiederlo in chat all'assistente** ("porta heretic a 32K") — ha gli
   strumenti per modificare il file, llama-swap ricarica da solo
2. **pannello web** su `127.0.0.1:8012/ui`: log dal vivo, caricamento manuale
3. modificare a mano il file, commentato in italiano

## Temi e aspetto

Tema non sta in Settings: modale a parte (icona in sidebar, o `/theme`).
Variabili CSS su `document.documentElement`, dichiarate in `static/style.css`.
Fondamentali: `--bg`, `--fg`, `--panel`, `--border`, `--red`.

16 temi predefiniti, massimo 8 personalizzati. Persistenza in localStorage con
copia sul server: tema **per utente**.

Font personalizzati: file in `static/fonts/custom/`.

**CSS o JavaScript personalizzati: non supportati.** Nessun meccanismo di
iniezione. Per andare oltre le variabili bisogna editare `static/style.css`,
modificare il core (fatto per l'avatar).

L'agente può cambiare tema da solo: azioni `set_theme` e `create_theme`.

**Appearance** in Settings è un'altra cosa: solo visibilità elementi
interfaccia. Lì abbiamo aggiunto le nostre due schede.
...
- `agent_input_token_budget` e `agent_input_token_hard_max` **senza controlli
...
- **Voce**: scheda TTS nascosta di proposito, codice STT punta a elementi
  inesistenti. Configurabile solo via API o file. (Pulsante di accensione
  riabilitato da noi.) Menu a tendina della trascrizione **non conosce** il
  valore `stream`: sceglierne un altro da lì lo sovrascriverebbe.
- Impostazioni skill nella modale Brain scrivono in `user_prefs.json` e
  **vincono** sulle globali.
- Nella cartella `docs/` originale nessuna documentazione sui temi.
...
Avviarli **staccati** (`Start-Process -WindowStyle Hidden`) per sopravvivere
alla chiusura del terminale.

### Per spegnere ShadowBroker

Non basta uccidere `main.py`: uvicorn con reload lascia figli
`multiprocessing.spawn` in ascolto sulla porta, Windows lo permette senza
