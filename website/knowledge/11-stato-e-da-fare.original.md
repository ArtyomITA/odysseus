...
- La scelta del modello dal menu, e il cambio di profilo
- La vista: caricare un'immagine e chiedere cosa c'è
- L'avatar con le emozioni: serve applicare una personalità e **ricaricare**
- La voce: menu `+` → Voce, poi scrivere qualcosa
- **La mappa che reagisce**: chiedere "cosa succede in Ucraina" col pannello
  aperto e vedere se si centra e filtra da sola
- **Le evidenziazioni**: gli anelli arancioni sulle entità citate, che svaniscono
  dopo 120 secondi
- **La voce senza pause** sulle risposte lunghe
- **Il microfono in diretta**: premere Voce e parlare — il testo deve comparire
  mentre parli e fermarsi da solo quando taci
...
Obiettivo: restare sotto i **3.500 token per osservazione**. Sotto quella soglia
...
Poi tre righe di instradamento nel prompt di sistema (quale strumento per quale
compito).
...
Il piano originale lo prevedeva e non l'abbiamo mai fatto: **30-50 richieste reali**
con risposte di riferimento, per misurare ogni modifica invece di andare a
sensazione. Metriche: percentuale di chiamate a strumenti ben formate, correttezza
...
Oggi l'indice di **tutte** le skill entra in ogni prompt. Con l'embedding su CPU
(costo VRAM zero) si iniettano solo quelle rilevanti.
...
- `llama-swap` gira con `--watch-config`: modificando il suo YAML le modifiche si
  applicano senza riavvio.
