...
Nel menu di Odysseus ci sono quattro voci, ma **non sono quattro modelli**: sono
quattro **profili di llama-swap** su un solo indirizzo (`127.0.0.1:8012`).
...
Le due righe "vista" caricano **gli stessi pesi** delle righe sopra. Cambia solo
il proiettore e, di conseguenza, il contesto (il proiettore mangia VRAM anche
stando su RAM di sistema).

### Risposta alla domanda "serve anche per la vista?"

**Sì, ed è esattamente la stessa cosa.** Non esiste un interruttore vista sì/no
sullo stesso processo: `--mmproj` è un argomento della riga di comando di
`llama-server`, deciso all'avvio. Passare da `qwenpaw` a `qwenpaw-vista`
significa che llama-swap **spegne un processo e ne accende un altro**, esattamente
come passare a `heretic`. Stesso tempo, stesso caricamento, stesso spinner.

Misurato su questa macchina: **~50 secondi a freddo**, pochi secondi se il file
è già nella cache del disco.

---

## 2. Le API di llama-swap che rendono possibile lo spinner

Scoperte provando sul server acceso, non documentate nel nostro materiale
precedente:
...
backend la lancia su un thread separato e non la aspetta.
...
**`src/llamaswap.py`** (nuovo). Tre livelli di cache, perché le tre informazioni
cambiano a ritmi diversi:
...
Odysseus, perché `/running` e `/upstream` stanno **un livello sopra**
l'API OpenAI.
...
così com'è**: sarebbe una falla SSRF (il server andrebbe a bussare a qualunque
indirizzo raggiungibile dalla macchina). `_resolve_known_endpoint()` accetta solo
URL che corrispondono a un endpoint **già configurato e abilitato**, visibile a
chi chiede, e usa poi l'URL preso dal database.
...
- l'overlay compare solo dopo **350 ms**: uno scambio a caldo non deve produrre
  un lampo di spinner
- spinner **whirlpool nativo** di Odysseus (`spinner.createWhirlpool(44)`), le
  stesse variabili CSS del resto (`--bg`, `--border`, `--fg`)
- contatore dei secondi; a 75 secondi il sottotitolo diventa "più lento del
...
- un `token` progressivo: se cambi modello due volte di fila, il primo giro
  esce in silenzio invece di litigare per lo schermo sopra al secondo

L'aggancio è in `modelPicker.js`, funzione `_pick()`, su **entrambe** le uscite
riuscite (chat non ancora creata / sessione esistente). Volutamente **non
aspettato**: il menu resta reattivo, e llama-swap accoda comunque una richiesta
...
I nostri profili si chiamano `qwenpaw-vista` e `heretic-vista`. **Nessuna parola
chiave corrisponde.** Conseguenza: anche con il proiettore caricato, Odysseus
considerava il modello cieco, **toglieva l'immagine dalla richiesta** e la
mandava a un "modello vista" separato configurato in Impostazioni — che da noi
non esiste, quindi il risultato era il testo
...
`supports_vision()` torna `None`, non `False`, quando non sa: un endpoint che non
è llama-swap si comporta esattamente come prima.
...
`model_supports_vision()`, che ricade sull'euristica del nome e quindi rispondeva
...
passa `load=True`: il profilo sta comunque per essere avviato dalla richiesta in
uscita, quindi l'attesa è la stessa attesa, solo anticipata abbastanza da
...
I due proiettori di QwenPaw erano **file vuoti**, residuo di un download fallito
del 23 luglio. `qwenpaw-vista` non era mai partito, e nessuno se n'era accorto
perché fino a oggi nessuna parte del sistema tentava di caricarlo.

Riscaricato da `mradermacher/QwenPaw-Flash-9B-GGUF` (repo **statico**, non `i1`:
i proiettori non stanno nel repo imatrix) con aria2:
...
Il file `mmproj-f16.gguf` resta cancellato: usiamo il Q8_0, e comunque sta su RAM
di sistema (`--no-mmproj-offload`).

**Morale:** un download fallito lascia un file da 0 byte che sembra presente in
`ls`. Il controllo giusto sono i **primi 4 byte**, che per un GGUF valido sono
`47 47 55 46`.
...
`src/agent_loop.py`, funzione `_drop_image_only_tools()`, applicata dopo tutta la
selezione degli strumenti (subito dopo `_expand_browser_mcp_tools`).
...
llama-swap, profilo non ancora caricato — gli strumenti restano tutti. Un dubbio
non deve accecare il modello.

Cosa **non** viene tolto, di proposito:

- `Snapshot` di Windows-MCP: legge l'albero di accessibilità, è **testo**, ed è
  anche più economico di una foto
...
modello (`.model-blind-badge`), con la spiegazione nel suggerimento al passaggio
del mouse. Compare **solo** quando la capacità è nota e negativa: mai su un
modello di cui non sappiamo nulla.
...
- La pastiglia si aggiorna quando cambi modello o al caricamento pagina. Se
  llama-swap fa scadere il profilo per conto suo (ttl 3600) l'interfaccia non se
  ne accorge finché non ricarichi. Ininfluente: le capacità del profilo non
  cambiano, solo il suo stato.
- Il pulsante di allegato immagine **non** è disabilitato sui profili ciechi.
  Volutamente: Odysseus ha comunque la via del "modello vista separato", che un
  domani potremmo configurare puntandolo su `qwenpaw-vista`. Sarebbe la soluzione
  elegante — descrivere l'immagine con il profilo vista senza scaricare quello
  con cui stai chattando. **Da valutare**, vedi
