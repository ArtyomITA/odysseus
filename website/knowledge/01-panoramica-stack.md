...
Vergilius è fork personale di Odysseus, adattato per girare **interamente in
...
| 8012 | **llama-swap** | espone profili modello come modelli OpenAI | `scripts\start-llama-swap.ps1` |
| 8013 | **ponte voce** | traduce OpenAI ↔ PocketTTS, ospita **Nemotron-3.5** per trascrizione in diretta (WebSocket) | `scripts\start-voce.ps1` |
...
| — | **Windows-MCP** | 13 strumenti controllo PC (sfoltiti da 20) | sottoprocesso di Odysseus (stdio) |
...
sopravvivono a chiusura terminale e crash editor.
...
**Niente si accumula prima di essere consegnato.** In entrata microfono manda
100 ms alla volta; in uscita audio suona mentre viene ancora generato. Erano
quattro punti che raccoglievano tutto prima di passarlo, da soli valevano ~4
...
1. **Tutto in locale.** Nessun dato esce dalla macchina, tranne ricerche web e
   (se scelto) voci cloud di Edge.
2. **Niente Docker.** Scelta deliberata: su Windows costerebbe macchina
   virtuale da 2GB di RAM che non abbiamo.
3. **La GPU è solo del modello.** Voce, trascrizione, embedding e proiettore
   visivo girano tutti su CPU. Nemotron-3.5 in streaming rispetta regola: 680
