...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume Vergilius owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch with Vergilius’ venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel.',
    fixes: [
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch from the cached local snapshot path. Vergilius now rewrites MLX repo-id launches to the newest local Hugging Face snapshot when it exists on the selected Mac.',
    fixes: [
      { label: 'Edit / relaunch serve', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('mlx_lm') },
      { label: 'Copy error', action: (panel) => {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## Vergilius Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
