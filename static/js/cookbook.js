  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Vergilius.');
    ...
    const _appDepsHtml = (items) => {
      ...
      const parts = [_sectionHeader('Vergilius app', 'Run inside the Vergilius app itself.')];
      const order = ['System', 'Tools', 'Image', 'LLM', 'Audio', 'Other'];
      for (const cat of order) {
        const catItems = _sortDeps(byCat.get(cat) || [], cat);
        if (!catItems.length) continue;
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. Vergilius will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Vergilius capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Vergilius keys, choose model directories, and set the default server. Local is this machine.</p>';
